...
export const api = {
  ...
  cloneEntry: (slug: string, id: string) =>
    req<Entry>(`/api/${slug}/${id}/clone`, { method: 'POST' }),

  bulkUpdateStatus: (slug: string, ids: string[], status: EntryStatus) =>
    req<{ succeeded: string[]; failed: Array<{ id: string; error: string }> }>(
      `/api/${slug}/bulk`,
      {
        method: 'PATCH',
  ...
  importEntries: (
    ...
    req<{ created: string[]; failed: Array<{ index: number; error: string }> }>(
      `/api/${slug}/import`,
      {
        method: 'POST',
